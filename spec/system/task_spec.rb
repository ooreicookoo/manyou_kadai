...
RSpec.describe 'タスク管理機能', type: :system do
  ...
  describe 'タスクを終了期限でソートしたら昇順に並んでいる場合' do
    it '終了期限が近いタスクが1番上に表示される' do
      click_on '終了期限'
      task_list = all('.task_row_limit_date')
      expect(task_lits[0].to have_content '2020')
      expect(task_lits[1].to have_content '2021')
      expect(task_lits[2].to have_content '2022')
    end
  end
